...
namespace FclEx.Http.SocksUtil
{
	internal static class Util
	{
		...
		internal static void ValidateConnectToDestinationResponse(byte[] receiveBuffer, int receiveCount)
		{
			if (receiveCount < 7)
			{
				throw new SocksException($"The SOCKS5 proxy responded with {receiveCount} bytes to the connect request. At least 7 bytes are expected.");
			}
			...
			{
				throw new SocksException($"The SOCKS5 proxy responded with a unsuccessful reply type '{(receiveBuffer[1] >= (byte)ReplyType.Unassigned ? ReplyType.Unassigned : (ReplyType)receiveBuffer[1])}' (0x{receiveBuffer[1]:x2}).");
			}
			if (receiveBuffer[2] != 0x00)
			{
				throw new SocksException($"The SOCKS5 proxy responded with an unexpected reserved field value 0x{receiveBuffer[2]:x2}. 0x00 was expected.");
			}
			if (!Enum.GetValues(typeof(AddressType)).Cast<byte>().Contains(receiveBuffer[3]))
			{
				throw new SocksException($"The SOCKS5 proxy responded with an unexpected {nameof(AddressType)} 0x{receiveBuffer[3]:x2}.");
			}
			...
			if (bindAddressType == AddressType.IpV4)
			{
				if (receiveCount != 10)
				{
					throw new SocksException($"The SOCKS5 proxy responded with an unexpected number of bytes ({receiveCount} bytes) when the address is an IPv4 address. 10 bytes were expected.");
				}
			...
			else if (bindAddressType == AddressType.IpV6)
			{
				if (receiveCount != 22)
				{
					throw new SocksException($"The SOCKS5 proxy responded with an unexpected number of bytes ({receiveCount} bytes) when the address is an IPv6 address. 22 bytes were expected.");
				}
		...
		internal static void ValidateHandshakeResponse(byte[] receiveBuffer, int receiveCount)
		{
			if(receiveCount != 2)
			{
				throw new SocksException($"The SOCKS5 proxy responded with {receiveCount} bytes, instead of 2, during the handshake.");
			}
			...
			{
				throw new SocksException("The SOCKS5 proxy does not support any of the client's authentication methods.");
			}
		}
		...
		private static void ValidateSocksVersion(byte version)
		{
			...
			{
				throw new SocksException($"The SOCKS5 proxy responded with 0x{version:x2}, instead of 0x{SocksVersion:x2}, for the SOCKS version number.");
			}
		}
