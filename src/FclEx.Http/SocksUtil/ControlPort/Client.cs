...
namespace FclEx.Http.SocksUtil.ControlPort
{
	public sealed class Client
	{
		...
		public async Task<bool> IsCircuitEstabilishedAsync(CancellationToken ctsToken = default(CancellationToken))
		{
			...
			else throw new SocksException($"Wrong response to 'GETINFO status/circuit-established': '{response}'");
		}
		...
		public async Task ChangeCircuitAsync(CancellationToken ctsToken = default(CancellationToken))
		{
			...
			{
				throw new SocksException("Couldn't change circuit", ex);
			}
			finally
			{
		...
		public async Task<string> SendCommandAsync(string command, bool initAuthDispose, CancellationToken ctsToken = default(CancellationToken))
		{
			try
			{
				...
				{
					throw new SocksException($"Failed to send command to TOR Control Port: {nameof(command)} : {command}", ex);
				}
		...
		public void DisconnectDisposeSocket()
		{
			...
			{
				throw new SocksException("Couldn't properly disconnect from the TOR Control Port.", ex);
			}
			finally
			{
		...
		public async Task InitializeConnectSocketAsync(CancellationToken ctsToken)
		{
			...
			{
				throw new SocksException("Couldn't connect to the TOR Control Port.", ex);
			}
		}
	}
}