    ...
    internal static class AutoMapperConfigurationExtensions
    {
        ...
        public static void CreateAutoAttributeMaps(this IMapperConfigurationExpression configuration, Type type, Type[] targetTypes, MemberList memberList)
        {
            ...
            foreach (var targetType in targetTypes)
            {
                ...
                var sourceParameterExpression = Expression.Parameter(type, "source");
                //In a lambda expression represent the target exemple : (target) => ...
                var targetParameterExpression = Expression.Parameter(targetType, "target");


                //We could use multiple AutoMapKey to compare the determine equality
                foreach (var propertyInfo in sourceKeysPropertyInfo)
                {
                    ...
                    var targetPropertyExpression = Expression.Property(targetParameterExpression, targetPropertyInfo);
                    ...
                    var equal = Expression.Equal(sourcePropertyExpression, targetPropertyExpression);

                    equalityComparer = equalityComparer is null 
                        ? equal 
                        : Expression.And(equalityComparer, equal);
                }
